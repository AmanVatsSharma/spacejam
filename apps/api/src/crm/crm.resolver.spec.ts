/**
 * File:        apps/api/src/crm/crm.resolver.spec.ts
...
function makeLead(overrides?: Partial<LeadEntity>): LeadEntity {
  return {
    id: 'lead-uuid',
    name: 'Test Lead',
...
function buildMockRepo(seeds: LeadEntity[] = []) {
  const data = [...seeds];
  return {
    create: vi.fn((dto: Partial<LeadEntity>) => makeLead(dto)),
    save: vi.fn(async (entity: LeadEntity) => {
    ...
    find: vi.fn(async (opts?: any) => {
      ...
      if (opts?.order) {
        results.sort((a: LeadEntity, b: LeadEntity) =>
    ...
    update: vi.fn(async (id: string, dto: Partial<LeadEntity>) => {
...
describe('Lead Entity', () => {
  ...
  it('LeadStatus enum should have 5 values', () => {
    expect(Object.values(LeadStatus)).toEqual([
      'New', 'Visited', 'Negotiation', 'Converted', 'Cold',
    ]);
  });

  it('LeadSource enum should have 5 values', () => {
    expect(Object.values(LeadSource)).toEqual([
      'Website', 'Referral', 'Walk-in', 'Social', 'Email',
    ]);
  });
  ...
  it('should support all optional fields', () => {
    const lead = makeLead({
      ...
      company: 'BigCorp',
      requirement: 'Enterprise suite',
      budget: '₹2,00,000/month',
      location: 'Hyderabad',
      notes: 'VIP — follow up tomorrow',
    ...
    expect(lead.requirement).toBeDefined();
    expect(lead.budget).toBeDefined();
    expect(lead.location).toBeDefined();
    expect(lead.notes).toBeDefined();
    expect(lead.assignedToId).toBe('user-42');
    expect(lead.centerId).toBe('center-7');
    expect(lead.lastContact).toBe('2025-06-15');
  });
});
...
describe('CrmResolver', () => {
  ...
  describe('Query.leads', () => {
    ...
    it('should combine multiple filters', async () => {
      ...
      expect(leads).toHaveLength(2);
      expect(leads.every(l => (l as any).status === LeadStatus.NEW && (l as any).source === LeadSource.WEBSITE)).toBe(true);
    });
  });
