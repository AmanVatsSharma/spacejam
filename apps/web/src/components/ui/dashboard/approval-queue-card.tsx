    ...
    <div className={`bg-white rounded-[19px] shadow-[0px_0px_6px_rgba(252,135,88,0.2)] flex flex-col p-5 ${className}`}>
      ...
      <div className="flex flex-col gap-3">
        {defaultItems.map((item) => (
          <div
            key={item.id}
            className="flex items-center gap-3 p-3 rounded-[16px] bg-gray-50 cursor-pointer hover:bg-gray-100 transition-colors"
          >
            {/* Icon */}
            <div
      ...
      {onViewAll && (
        <button
          onClick={onViewAll}
          className="mt-4 w-full h-[40px] bg-white border border-[#E5E7EB] rounded-xl flex items-center justify-center gap-2 text-[14px] font-medium text-[#6B7280] hover:bg-gray-50 transition-colors"
        >
          View All Approvals
        </button>
      )}
