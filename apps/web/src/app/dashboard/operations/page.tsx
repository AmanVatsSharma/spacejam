    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="bg-white rounded-2xl shadow-sm p-6 flex items-center justify-between">
        <div className="flex flex-col gap-1">
          ...
          <p className="text-[#4A5565]">
            Manage bookings, check-ins, and space utilization
          </p>
        </div>
      ...
      <div className="grid grid-cols-4 gap-4">
        <div className="bg-white rounded-2xl shadow-sm p-5 transition-all duration-200 hover:shadow-md hover:-translate-y-0.5">
          ...
          <p className="text-2xl font-bold text-[#101828]">
            {metricsLoading
              ? '—'
              : (metrics?.activeBookings ?? activeBookings.length)}
          </p>
        ...
        <div className="bg-white rounded-2xl shadow-sm overflow-hidden transition-all duration-200">
          ...
          <table className="w-full">
            <thead className="bg-gray-50">
              <tr>
                <th className="px-6 py-3 compact:px-3 compact:py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                  Booking ID
                </th>
                <th className="px-6 py-3 compact:px-3 compact:py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                  Guest
            ...
            <tbody className="divide-y divide-gray-100">
              {bookingsLoading && bookings.length === 0 ? (
                ...
                bookings.map((booking: any) => {
                  const statusInfo = BACKEND_STATUS_MAP[
                    booking.status as BackendBookingStatus
                  ] ?? {
                  ...
                  return (
                    ...
                    >
                      <td className="px-6 py-4 compact:px-3 compact:py-2 text-sm font-medium text-[#101828]">
                        {booking.id.slice(0, 8)}…
                      </td>
                      <td className="px-6 py-4 compact:px-3 compact:py-2 text-sm text-[#4A5565]">
                        {booking.user?.name ?? 'Unknown'}
                      </td>
                      <td className="px-6 py-4 compact:px-3 compact:py-2 text-sm text-[#101828] font-medium">
                        {booking.seat?.name ?? booking.meetingRoom?.name ?? '—'}
                      </td>
                      <td className="px-6 py-4 compact:px-3 compact:py-2 text-sm text-[#4A5565]">
                      ...
                      <td className="px-6 py-4 compact:px-3 compact:py-2 flex gap-2">
                        {booking.status === 'CONFIRMED' && (
                          <button
                            ...
                            className="text-xs bg-green-500 text-white px-3 py-1 rounded-lg hover:bg-green-600 transition-all active:scale-[0.97]"
                          >
                            Check In
                          </button>
                        )}
                        {booking.status === 'CHECKED_IN' && (
                          <>
                            <button
                              ...
                              className="text-xs bg-blue-100 text-blue-600 px-3 py-1 rounded-lg hover:bg-blue-200 mr-1 transition-all active:scale-[0.97]"
                            >
                              Extend
                            </button>
                            <button
                              onClick={() => handleCheckOut(booking.id)}
                              className="text-xs bg-orange-500 text-white px-3 py-1 rounded-lg hover:bg-orange-600 transition-all active:scale-[0.97]"
                            >
                              Check Out
                            </button>
                          </>
                        ...
                        {(booking.status === 'CONFIRMED' ||
                          ...
                          <button
                            ...
                            className="text-xs bg-red-100 text-red-600 px-3 py-1 rounded-lg hover:bg-red-200 transition-all active:scale-[0.97]"
                          >
                            Cancel
                          </button>
                        )}
        ...
        <div className="grid grid-cols-1 compact:grid-cols-2 gap-6 compact:gap-3">
                ...
                bookings
                  ...
                  .map((booking: any) => {
                    ...
                    return (
                      <div
                        key={booking.id}
                        className="flex items-center justify-between p-4 bg-gray-50 rounded-xl transition-all duration-200 hover:shadow-sm hover:-translate-y-0.5"
                      >
                        <div>
                        ...
                        <button
                          onClick={() => handleCheckIn(booking.id)}
                          className="bg-green-500 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-green-600 transition-all active:scale-[0.97]"
                        >
                          Check In
                        </button>
                      </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm p-6 transition-all duration-200 hover:shadow-md">
            ...
            <div className="space-y-4">
              {bookingsLoading ? (
                <div className="text-sm text-gray-400">Loading…</div>
              ) : bookings.filter((b: any) => b.status === 'CHECKED_IN')
                  .length === 0 ? (
                <div className="text-sm text-gray-400">
                  ...
                  .map((booking: any) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <button
                          onClick={() => handleCheckOut(booking.id)}
                          className="bg-orange-500 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-orange-600 transition-all active:scale-[0.97]"
                        >
                          Check Out
                        </button>
                      </div>
      ...
      {activeTab === 'spaces' && (
        <div className="bg-white rounded-2xl shadow-sm p-6 transition-all duration-200 hover:shadow-md">
          ...
          <div className="grid grid-cols-4 gap-4">
            {['Cabins', 'Hot Desks', 'Meeting Rooms', 'Dedicated Desks'].map(
              (type, i) => {
                const counts: Record<string, number> = {
                  Cabins: metrics?.totalSeats
                    ? Math.floor(metrics.totalSeats * 0.25)
                    : 0,
                  'Hot Desks': metrics?.totalSeats
                    ? Math.floor(metrics.totalSeats * 0.4)
                    : 0,
                  'Meeting Rooms': metrics?.availableSeats
                    ? Math.floor(metrics.availableSeats * 0.15)
                    : 0,
                  'Dedicated Desks': metrics?.totalSeats
                    ? Math.floor(metrics.totalSeats * 0.2)
                    : 0,
                };
                return (
                  <div
                    key={type}
                    className="bg-gray-50 rounded-xl p-4 text-center transition-all duration-200 hover:shadow-md hover:-translate-y-0.5"
                  >
      ...
      {activeTab === 'meeting-rooms' && (
        <MeetingRoomsTab onBookRoom={() => setShowBookRoom(true)} />
      )}
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-4 gap-4">
        ...
        <div className="bg-white rounded-2xl shadow-sm p-5 transition-all duration-200 hover:shadow-md hover:-translate-y-0.5">
          ...
          <p className="text-2xl font-bold text-[#101828]">
            {roomsLoading ? '—' : occupiedCount}
          </p>
        </div>
        <div className="bg-white rounded-2xl shadow-sm p-5 transition-all duration-200 hover:shadow-md hover:-translate-y-0.5">
          <p className="text-sm text-gray-500 mb-2">Vacant Slot</p>
          <p className="text-2xl font-bold text-[#101828]">
            {roomsLoading ? '—' : availableCount}
          </p>
        </div>
        <div className="bg-white rounded-2xl shadow-sm p-5 transition-all duration-200 hover:shadow-md hover:-translate-y-0.5">
      ...
      <div className="grid grid-cols-1 compact:grid-cols-2 gap-6 compact:gap-3">
        {roomsLoading ? (
          ...
          rooms.map((room: any) => {
            const style =
              ROOM_STATUS_STYLE[room.status] ?? ROOM_STATUS_STYLE['AVAILABLE'];
            const isActionable =
            ...
            return (
              ...
              >
                <div className="flex items-start justify-between mb-4">
                  ...
                  <span
                    className={`px-3 py-1 rounded-full text-xs font-medium ${style.pill}`}
                  >
                    {style.label}
                  </span>
                </div>
                ...
                {isActionable ? (
                  ...
                  <button
                    className="w-full py-2.5 rounded-xl text-sm font-medium border border-gray-200 text-[#4A5565] opacity-50 cursor-not-allowed transition-all"
                    disabled
                  >
                    Unavailable
                  </button>
