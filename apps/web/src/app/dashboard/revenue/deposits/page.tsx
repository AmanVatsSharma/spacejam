                ...
                {loading && filtered.length === 0 ? (
                  <tr>
                    <td colSpan={7} className="px-6 py-12">
                      <QueryLoading message="Loading deposits…" />
                    </td>
                  </tr>
                ) : error && filtered.length === 0 ? (
                  <tr>
                    <td colSpan={7} className="px-6 py-12">
                      <QueryError message="Unable to load deposits." onRetry={() => refetch()} />
                    </td>
                  </tr>
                ) : filtered.length === 0 ? (
                  <tr>
                    <td colSpan={7} className="px-6 py-12">
                      <QueryEmpty message="No deposits found" hint="Add a new deposit to get started." />
                    </td>
                  </tr>
                ) : (
                    ...
                    <tr key={deposit.id} className="transition-colors duration-150 hover:bg-[#F9FAFB]">
                      ...
                      <td className="px-6 py-5 text-center relative">
                        <button
                          ...
                          disabled={loading}
                          className="text-gray-400 hover:text-gray-600 focus:outline-none p-1 rounded-full hover:bg-gray-100 transition-all active:scale-[0.97] transition-transform duration-150 disabled:opacity-50 disabled:cursor-not-allowed"
                        >
                          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      ...
      <div className="w-80 shrink-0 flex flex-col gap-6">
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
          ...
          <div className="flex flex-col gap-3">
            <button disabled={loading} className="flex items-center gap-3 px-4 py-3.5 border border-gray-200 rounded-xl hover:bg-gray-50 hover:border-gray-300 transition-all active:scale-[0.97] transition-transform duration-150 text-sm font-semibold text-gray-700 disabled:opacity-50 disabled:cursor-not-allowed" onClick={() => setIsApproveModalOpen(true)}>
            ...
            <button disabled={loading} className="flex items-center gap-3 px-4 py-3.5 border border-gray-200 rounded-xl hover:bg-gray-50 hover:border-gray-300 transition-all active:scale-[0.97] transition-transform duration-150 text-sm font-semibold text-gray-700 disabled:opacity-50 disabled:cursor-not-allowed" onClick={() => setIsFreezeModalOpen(true)}>
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="text-gray-400">
                <path d="M12 2v20M2 12h20M4.93 4.93l14.14 14.14M19.07 4.93L4.93 19.07" />
              </svg>
              Freeze Account
            </button>
            <button disabled={loading} className="flex items-center gap-3 px-4 py-3.5 border border-gray-200 rounded-xl hover:bg-gray-50 hover:border-gray-300 transition-all active:scale-[0.97] transition-transform duration-150 text-sm font-semibold text-gray-700 disabled:opacity-50 disabled:cursor-not-allowed" onClick={() => setIsAddDepositModalOpen(true)}>
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="text-gray-400">
